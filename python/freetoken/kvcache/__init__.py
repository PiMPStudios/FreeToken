...
def create_kv_pool(config, num_pages: int, device: torch.device, dtype: torch.dtype):
    ...
    spec_tokens = 0
    if getattr(config, "experimental_mtp", False):
        spec_tokens = max(0, int(getattr(config, "experimental_mtp_tokens", 1) or 1))
...
def create_kvcache_pool(
    ...
    if len(kv_specs) == 1 and kv_specs[0].attn_type == _AttnType.QSA:
        ...
        num_global_layers = max(model_config.num_layers, max(spec.layer_ids) + 1)
        return QSAKVCache(
            num_kv_heads=spec.num_kv_heads,
